...
function make_videos_csv() {
  ...
  find "$dataset_path" -mindepth 4 -maxdepth 4 -type f -name 'Labels-v2.json' \
    ...
    | while read json_fpath; do
        ...
        first_half_duration_sec=`video_time_duration "$dataset_path"/"$match_path"/1_720p.mkv`
        second_half_duration_sec=`video_time_duration "$dataset_path"/"$match_path"/2_720p.mkv`
